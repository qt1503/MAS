...
class CoTTesting:
    ...
    def testing(self, name: str):
        ...
        self.all_results = []  # Initialize all_results as instance variable
        ...
        def compare_result(inputs: dict, reference_outputs: dict, outputs: dict):
            ...
            self.all_results.append({
                "question": inputs["question"],
                "steps": [step.dict() for step in outputs["steps"]] if outputs.get("steps") else [],
                "true_answer": actual,
        ...
        try:
            ...
            examples = list(client.list_examples(dataset_name=f"{name_dataset}"))
            ...
            if total > 0:
                ...
                with open(f"save_log/CoT_results_{self.name}_{datetime.now().strftime('%d-%m-%Y %H:%M:%S')}_{total}samples.json", "w", encoding="utf-8") as f:
                    json.dump(summary, f, ensure_ascii=False, indent=2)

                print(f"✅ Kết quả đã được lưu vào save_log/CoT_results - {self.name}__{datetime.now().strftime('%d-%m-%Y %H:%M:%S')}_{total}samples.json")
